    ...
    public static function initProcess()
    {
        ...
        if (!empty($data)) {
            ...
            if (function_exists('wp_verify_nonce')) {
                if (isset($data['mktr']['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($data['mktr']['_wpnonce'])), 'save-config')) {
                    $fail = array('tracking' => false, 'google' => false);
                    $onboarding = false;
                    $push_status = true;
                    foreach ($data[Config::$name] as $key => $value) {

                        if (in_array($key, array('tracking_key', 'rest_key', 'customer_id')) && empty($value)) {
                            $fail['tracking'] = true;
                        }
                        if (in_array($key, array('google_tagCode')) && empty($value) && isset($data[Config::$name]['google_status']) && $data[Config::$name]['google_status'] == 1) {
                            $fail['google'] = true;
                        }
                        if (in_array($key, array('update_feed', 'update_review')) && empty($value)) {
                            $value = 4;
                        }
                        if (in_array($key, array('allow_export_gravity_data'))) {
                            $value = serialize($data[Config::$name]['allow_export_gravity_data']);
                        }
                        if ($key !== 'onboarding') {
                        ...
                        if ($key == 'opt_in') {
                            ...
                            if ($active) {
                                $stat = \MailPoet\Settings\SettingsController::getInstance()->get('signup_confirmation.enabled');

                                if (Config::getValue('opt_in_oldmail') === null || $stat != 0) {
                                    Config::setValue('opt_in_oldmail', $stat);
                                }
                                ...
                                if ($id !== null) {
                                    ...
                                    foreach ($f->findAll() as $ff) {
                                        ...
                                        if (!in_array($id, $settings['segments'])) {
                                            $settings['segments'][] = (string)$id;
                                            $ff->setSettings($settings);
                                            try {
                                                $f->persist($ff);
                                                $f->flush();
                                            } catch (\Exception $e) {
                    ...
                    Admin::addNotice(array('type' => 'error', 'message' => 'Invalid request, settings have not been saved.'));
                }
            }
        }
    }
